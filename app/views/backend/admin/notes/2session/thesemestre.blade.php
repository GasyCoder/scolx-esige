...
@section('title') Notes 2e Session @stop
@section('content')
<!-- Workspace -->
...
<div class="container flex items-center justify-center py-10">
        ...
        @include('backend.admin.administration.Pay.return')    
            <h3><span class="la la-check-square text-xl leading-none"></span>Choisir semestre</h3>
                     ...
                     <select name="type" id="type" onchange="goToNext(this)" class="form-control">
                            <option value="" selected disabled>-- Choisir semestre --</option>
                              @foreach($semestres as $semestre)
                                    <option value="{{ $semestre->codeSem}}">{{ $semestre->semestre }}</option>
                              @endforeach
                        </select>
