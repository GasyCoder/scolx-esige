...
$delibera = Deliberation::where('class_id', $class->id)
                        ->where('parcour_id', $parcour->id) 
                        ->first();
?>
@section('title') Déliberation Semestre 1 @stop
@section('content')
<!-- Workspace -->
...
<div class="container flex items-center justify-center py-10">
        <div class="w-full md:w-1/2 xl:w-1/3">
            <div class="card p-5">
                <h3>DELIBERATION SEMESTRE 1</h3>
                <div class="mt-5">
                   {{ Form::open(['route'=>['submitDeliber', $class->id, $parcour->id], 'class'=>'flex-shrink flex-wrap', 'id'=>'', 'data-toggle'=>''])  }} 
                    <div class="mb-5">
                            <label class="label block mb-2" for="title">Moyenne déliberé</label>
                            <input type="number" class="form-control" value="{{$control->deliberation}}" disabled>
                    </div>
                    @foreach($delibers as $deliber) 	
                     <?php      
                        $totalponder        = Credit::where('id_student',       $deliber->id_student)
                                                    ->where('class_id',         $deliber->class_id)
                                                    ->where('parcour_id',       $deliber->parcour_id)
                    ...
                    ?>
                        <div class="mb-5" style="display:none;">
                            <input type="number" class="form-control" name="deliberation[]" value="10">
                    ...
                    <div class="mt-10">
                            ...
                            <button type="submit" class="btn btn_primary"><i class="la la-check-square"></i> Confirmer  
                            </button>
                            @else
                            ...
                            <a onclick="return confirm('Vous êtes sur cette action?')" href="{{URL::route('restoreDeliber', [$class->id, $parcour->id])}}" class="btn btn_danger ltr:ml-3 rtl:mr-3"><i class="la la-trash"></i> Restaurer</a>
                            @endif
                    </div> 
                    {{ Form::close() }}
                </div>
