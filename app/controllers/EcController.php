...
class EcController extends BaseController {
	...
	public function index()
	{
		...
		$ues  	 	 	 = UE::all();
		$troncs  	 	 = Tronc::orderBy('id', 'asc')->get();
		$elementsC 		 = EC::orderBy('id', 'asc')->get();

		return View::make('backend.admin.pedagogie.EC.index', compact('elementsC', 'etudiants', 'parcours', 'ues', 'classes', 'troncs'))->with('title', $title)->with('sous', $sous);
	}
	...
	public function AddtroncCommun($class)
	{
		...
		return View::make('backend.admin.pedagogie.EC.addtronc', compact('class','allParcours', 'allClasses', 'allUes', 'semestres'))->with('title', $title)->with('sous', $sous);
	}
	...
	public function ec_parcour($class)
	{
		...
		return View::make('backend.admin.pedagogie.EC.parcours', compact('parcours', 'class', 'ues'))->with('title', $title)->with('sous', $sous);
	}
	...
	{	
		...
		return View::make('backend.admin.pedagogie.EC.addTeacher', compact('autreparcours', 'allmatieres', 'element', 'class', 'parcour', 'teachers'))->with('title', $title)->with('sous', $sous);
	} 
	...
	{	
		...
		return View::make('backend.admin.pedagogie.EC.update', compact('elements', 'class', 'parcour'))->with('title', $title)->with('sous', $sous);
	} 
	...
	{	
		...
		$semes   		= Sem::where('status', 1)->get();

		return View::make('backend.admin.pedagogie.EC.addnew', compact('ec_s1', 'ec_s2', 'class', 'parcour', 'ues', 'matiereMixte', 'NoMixte', 'semestres', 'semes', 'choixs', 'parcours'))->with('title', $title)->with('sous', $sous);
	}
	...
	public function checkElement($class, $parcour, $id)
	{
		...
		return View::make('backend.admin.pedagogie.EC.details', compact('class', 'unites', 'parcour', 'matieres', 'element', 'troncs', 'troncs_egale', 'teacher', 'unite'))->with('title', $title)->with('sous', $sous);
	}
	...
	public function print_element($class, $parcour)
	{
       ...
       $mat_print = EC::where('tronc', 0)->where('class_id', $class->id)
		...
		return View::make('backend.admin.pedagogie.EC.impression', compact('class', 'parcour', 'matires_S1', 'matires_S2', 'mat_print_mixte', 'mat_print'))->with('title', $title)->with('sous', $sous);
	}
	...
	public function purge($element_id)
	{
		
		$tronc = Tronc::find($element_id);
		...
		if ($tronc !== null) {
			...
			$path = Session::get('language');
			return Redirect::back()->with('success', ('Matière a été supprimé avec succès!'));
		} 
