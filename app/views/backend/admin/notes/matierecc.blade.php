...
?>
@section('title') Matières CC - {{$class->short}} @stop
@section('content')
        ...
        <div class="card p-5">
            <div class="overflow-x-auto">
                <table class="table-sorter table table-auto table_hoverable w-full">
                    ...
                    <?php 
                            $exites = Notecc::where('element_id',      $element->id)
                        ...
                        <tr>
                           ...
                           <td class="text-center"><span class="badge badge_outlined badge_primary"><b>{{$element->semestre}}</b></span></td> 
                            ...
                            <td class="text-center">
                                ...
                                @if ($exites > 0)    
                                <a href="{{ URL::current().'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.$element->codeEc}}/afficher/notes/cc" class="badge badge_success" style="color:#fff">
                                    <span class="la la-eye text-xl leading-none ltr:mr-2 rtl:ml-2"></span>
                                    Afficher
                                </a>
                                @else
                                <a href="{{ URL::current().'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.$element->codeEc}}/notes/ajoutercc" class="badge badge_primary" style="color:#fff">
