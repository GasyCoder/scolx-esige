...
@include('backend.break')
        ...
        <div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2 mt-2">  
            <a href="/admin/absences/etudiants/{{ $class->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=') }}" class="btn btn-icon btn-icon_large btn_outlined btn_primary">
            ...
            <div class="custom-select mt-0">
                      <select name="select_element_name" id="select_element_name" onchange="goToNext(this)" class="form-control">
                        ...
                        @foreach($elements as $element)
                          <option value="{{ URL::to('/admin/absences/etudiants_tronc').'/'.$element->class_id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=')}}">{{ $element->name }}</option>
                        @endforeach
                        </select>
                    <div class="custom-select-icon la la-caret-down"></div>
            </div>
            <div class="flex flex-wrap gap-2 ltr:ml-auto rtl:mr-auto">
                ...
                <a href="{{URL::route('printAll', $class->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($mode->dateStart), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($mode->dateEnd), '+/', '-_'), '='))}}" target="_blank" class="badge badge_secondary font-bold">
                        ...
                        <thead>
                            <tr class="text-xl"> 
                             <th colspan="7" style="text-align:center;"><span class="la la-calendar"></span> <code> {{ \Carbon\Carbon::parse($mode->dateStart)->format('d M')}} au {{ \Carbon\Carbon::parse($mode->dateEnd)->format('d M Y')}} </code><br>
                                ...
                                <h4><u>Matière tronc commun</u>: <span style="color:#000">{{$mat->name}}</span></h4>
                             </th>
                            </tr>
                            <tr style="color:#334155">
                                ...
                                <th class="ltr:text-left rtl:text-right" style="text-align:center;">Parcours</th>
