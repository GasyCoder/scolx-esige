...
@section('title') Gestion des absences @stop
@section('content')
...
@include('backend.pages.alerts')        
        <div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2 mt-2">  
            <a href="/admin/absences/etudiants/anne_preparatoire/{{ $class->id }}" class="btn btn-icon btn-icon_large btn_outlined btn_primary">
            ...
            <div class="custom-select mt-0">
                      <select name="select_element_name" id="select_element_name" onchange="goToNext(this)" class="form-control">
                        ...
                        @foreach($elements as $element)
                          <option value="{{ URL::to('/admin/absences/etudiants/anne_preparatoire').'/'.$element->class_id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=')}}/absence/ap">{{ $element->name }}</option>
                        @endforeach
                        </select>
                    <div class="custom-select-icon la la-caret-down"></div>
            </div>
        </div>
@if(count($studentsAp) > 0)        
        <div class="mt-5">
             ...
             <div class="breadcrumb breadcrumb_alt p-5">   
                ...
                <div class="card p-5 mt-5">
                    <table class="table-sorter table table_bordered w-full mt-0">
                        ...
                        <tbody>
                        @foreach($studentsAp as $key => $ap)        
                        <tr>
                                <td class="text-center">
                                <center>
                                <div class="avatar w-4 h-4">
                                   @if(!empty($ap->image))
                                    <?php echo HTML::image('uploads/profiles/students/'.$ap->image.'', '', ['class'=>'', 'width'=>'','height'=>'']) ?>
                                    @elseif($ap->sexe == 1)
                                ...
                                <td>
                                    <a href="{{URL::route('profileEtudiant', [$ap->class_id, $ap->parcour_id.'/'.rtrim(strtr(base64_encode($ap->id), '+/', '-_'), '=')])}}">
                                    {{$ap->fname}} {{$ap->lname}}
                                    </a>
                                </td>
                                <td class="text-center">
                                Année Préparatoire
                                <input type="hidden" name="matricule[]" value="{{ $ap->matricule }}">
                                </td>
                                <td class="text-center">
                                <label class="px-4" for="s{{ $key+1 }}">
                                <input type="checkbox" class="c9" id="s{{ $key+1 }}" name="status[]" value="1">
                                <span style="color:#059669">@if($ap->sexe == 1)Présent @else Présente @endif</span>
                                </label>
                                <label class="px-4" for="s{{ $key+1 }}">
                                <input type="checkbox" class="c6" id="s{{ $key+1 }}" name="status[]" value="0">
                                <span style="color:#C2410C">@if($ap->sexe == 1)Absent @else Absente @endif</span>
                                </label>
                                </td>
                            </tr>
                         @endforeach  

                    <!-- Etudiants Auditeur Libre -->

                         @foreach($mixtes as $key => $student)      
                            <tr>
                                <td class="text-center">
                                <center>
                                <div class="avatar w-4 h-4">
                   ...
                   <div class="mt-auto">
                    ...
                    <?php 
                        $x = Absence::where('class_id', $class->id)
                    ...
                    ?>
                    @if($x > 0)    
                        <a href="{{URL::route('listeAp', $class->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '='))}}" class="btn btn_success">    
    ...
    <aside class="sidebar">
        ...
        <div class="flex flex-col gap-y-5 p-5">
            <a href="#" class="flex items-center text-normal font-bold">
                ...
                Matière
                <span class="badge badge_danger ltr:mr-2 rtl:ml-2 ltr:ml-auto rtl:mr-auto">{{ $mat->name }}</span>
            </a>
        </div>
            ...
            <a href="#" class="flex items-center text-normal">
                ...
                <?php 
                    ...
                    $auditeurs   = Student::where('class_id', '!=', $class->id)
                                            ...
                                            ->count();
                ?>
                <span class="badge badge_primary ltr:mr-2 rtl:ml-2 ltr:ml-auto rtl:mr-auto">{{ ($students)+($auditeurs) }}</span>
            </a>
            <a href="#" class="flex items-center text-normal">
