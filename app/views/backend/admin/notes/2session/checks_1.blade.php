...
@section('title') Notes 2e Session - S_1 @stop
@section('content')
<style type="text/css">
...
<main class="workspace workspace_with-sidebar">  
                  ...
                  <select name="select_student_name" id="select_student_name" onchange="goToNext(this)" class="form-control">
                    <option value="" selected disabled>--Ajouter pour les autres étudiants--</option>
                        @foreach($students as $student)
                            ...
                            <option value="/admin/notes/etudiants/notes/ajouter-2e-session/{{$student->stud->class_id.'/'.$student->stud->parcour_id.'/1/'.rtrim(strtr(base64_encode($student->stud->id), '+/', '-_'), '=')}}">
                                {{ $student->stud->fname }} {{ $student->stud->lname }}
                            </option>
                            @endif
                        @endforeach
                    </select>
    ...
    {{ Form::open(['route'=>['resultat2Session_1', $class->id, $parcour->id], 'class'=>'flex-shrink flex-wrap', 'id'=>'', 'data-toggle'=>''])  }}
       @if($dsexist > 0) 
       @else
                    ...
                    <div class="accordion rounded-xl mt-0">
                        ...
                        <thead>
                            <tr style="color:#3F3F46">
                                ...
                                <th class="" style="text-align:center" width="50">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($ues_1 as $ue)
                        ...
                        ?>       
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="{{ URL::route('addNoteSession_1', [$class->id, $parcour->id]).'/'.rtrim(strtr(base64_encode($noty->stud->id), '+/', '-_'), '=').'/'.$noty->id}}" class="badge badge_primary ltr:ml-2 rtl:mr-2 mt-2">
 ...
 <table class="table table_bordered w-full" style="display:none;">
                    ...
                    @foreach($noteStuds as $noteStud)
                        <?php 
                            ...
                            $mynotes       = Note::where('class_id',            $class->id)
                                                        ->where('parcour_id',   $parcour->id)
                          ...
                          <?php $Somue = $ponder/$Coefe ?>  
                           <table style="display:"> 
                            ...
                            <table class="table_bordered" width="100%">   
                                ...
                                <input type="hidden" name="parcour_id[]" value="{{$parcour->id}}" class="form-control">
