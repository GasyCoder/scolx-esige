...
<div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2 mt-2">  
           <a href="/admin/notes/etudiants/{{$class->id.'/'.$parcour->id.'/'.$element->semestre.'/1/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.$element->codeEc}}/afficher/notes/examen" class="btn btn_outlined btn_primary">
    ...
    <div class="container flex items-center justify-center mb-2 py-1">
        <div class="card p-5 flex flex-col gap-y-2 w-full md:w-1/2 xl:w-1/4">
            ...
            {{ Form::open(['route'=>['updateNoteStd', $mark->id], 'class'=>'', 'id'=>'myForm2', 'data-toggle'=>'validator'])  }}   
                ...
                <div class="mb-5 w-full">
                <label class="label block mb-2" for="noteExam">Note</label>
                <input type="number" name="noteExam" value="{{$mark->noteExam}}" class="form-control">
                </div>     
                <div class="mt-5">
