...
<hr>
    ...
    <?php  
        ...
        $Coefs     = EC::where('codeUe', $rvnote->codeUe)
                                ->where('class_id', $rvnote->class_id)
                                ->where('parcour_id', $rvnote->parcour_id)
                                ->where('tronc', 0)
                                ->groupBy('codeEc')
        ...
        $CoefsFoix   = EC::where('codeEc', $rvnote->codeEc)
                                ->where('class_id', $rvnote->class_id)
                                ->where('parcour_id', $rvnote->parcour_id)
                                ->where('tronc', 0)
                                ->groupBy('codeUe')
        ...
        $ValCoefs     = EC::where('codeUe', $rvnote->codeUe)
                                ->where('class_id', $rvnote->class_id)
                                ->where('parcour_id', $rvnote->parcour_id)
                                ->where('tronc', 0)
                                ->groupBy('codeUe')
        ...
        $SumCoefs   = EC::where('class_id', $rvnote->class_id)
                                ->where('parcour_id', $rvnote->parcour_id)
                                ->where('tronc', 0)
                                ->groupBy('class_id')
                                ->sum('coef');
    ...
    ?>                  
      <tr>
        ...
        <td>
         <table class="table_bordered" width="100%">
         @foreach($Coefs as $coefe) 
          <tr style="line-height:12px;" align="center">
            <td>{{number_format($coefe->coef, 2, ',', '')}}</td>
          </tr>
          @endforeach  
        </table>
        </td>
        ...
        @foreach($Noty as $noty)
            <?php 
                $coeFa    = EC::where('id', $noty->element_id)
                                 ->where('class_id', $noty->class_id)
                                 ->where('parcour_id', $noty->parcour_id)
                                 ->where('codeUe', $noty->codeUe)
                                 ->where('codeEc', $noty->codeEc)
                                 ->groupBy('codeEc')
                                 ->first(); 
    ...
    <tfoot style="line-height:12px;">
      <tr>
        ...
        <td align="center"><b>{{number_format($totalNote, 2, ',', '')}}</b></td>
         <td align="center"><b>{{number_format($SumCoefs, 2, ',', '')}}</b></td>
         <td align="center">
            <b></b>
        </td>
        <td></td>
