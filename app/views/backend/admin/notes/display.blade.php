           ...
           <div class="custom-select mt-0">
                  <select name="select_element_name" id="select_element_name" onchange="goToNext(this)" class="form-control">
                    ...
                    @foreach($elements as $mat)
                        <option value="{{ URL::to('/admin/notes/etudiants').'/'.$mat->class_id.'/'.$mat->parcour_id.'/'.$mat->semestre.'/1/'.rtrim(strtr(base64_encode($mat->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($mat->codeEc), '+/', '-_'), '=').'/afficher/notes/examen'}}">{{ $mat->name }}</option>
                    @endforeach
                    </select>
                <div class="custom-select-icon la la-caret-down"></div>
            </div>
                    ...
                    <table class="table-sorter table table_bordered w-full mt-3">
                        <thead>
                            ...
                            <tr style="color:#334155">
                                ...
                                <th class="text-center" style="text-align:center;">Note Examen</th>
                        ...
                        <?php $student = Student::where('class_id', $mark->class_id)
                                                ...
                                                ->first(); 
                            $noty = Note::where('noteExam', $mark->noteExam)->first();
                        ...
                        ?>     
                            <tr>
                                ...
                                <td class="text-center">
                                  @if($mark->noteExam >= 10) 
                                   <span class="badge badge_success"> {{number_format($mark->noteExam, 2, ',', '')}}</span>
                                   @else
                                    <span class="badge badge_danger"> {{number_format($mark->noteExam, 2, ',', '')}}</span>
                                    @endif
                                </td>
                                @if(Auth::user()->is_admin) @if(!Auth::user()->is_secretaire)
                                <td class="text-center">
                                <div class="inline-flex ltr:ml-auto rtl:mr-auto">
                                    <a href="{{URL::route('modifierNote', [$mark->id, $mark->class_id, $mark->parcour_id.'/'.rtrim(strtr(base64_encode($mark->element_id), '+/', '-_'), '=')])}}" class="btn btn-icon btn_outlined btn_success ltr:ml-5 rtl:mr-5">
