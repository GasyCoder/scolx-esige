...
@section('title') Ajouter notes CC {{$class->short}} @stop
@section('content')
<style type="text/css">
                ...
                <div class="">  
                    <div class="mt-2">
                        <span class="badge badge_outlined badge_success font-bold">
                            ...
                            Matière: {{ $element->name }} ID = {{$ues->id}}
                        </span>
                    </div>
                </div>  
...
@if($fixes > 0)        
        <div class="mt-3">
            ...
            <input type="hidden" name="semestre" value="{{ $element->semestre }}">    
                    ...
                    <table class="table-sorter table table_bordered w-full mt-0">
                        <thead>
                            <tr style="color:#334155">
                                <th class="" style="text-align:center;" width="80">Photo</th>
                                <th class="ltr:text-left rtl:text-right" width="150">Nom complet</th>
                                <th class="" style="text-align:center;" width="50">Classe</th>
                                <th class="" style="text-align:center;" width="50">Parcour</th>
                                <th class="text-center" style="text-align:center;" width="80">Note CC1</th>
                                <th class="text-center" style="text-align:center;" width="80">Note CC2</th>
                            </tr>
                        </thead>
                        <tbody>
                         @foreach($students as $key => $student)
                            <tr>
                                ...
                                <input type="number" step="any" value="00.0" max="20" pattern="[0-9]+([\.][0-9]+)?" class="form-control" placeholder="Saisir le note CC sur 20" id="s{{ $key+1 }}" name="notecc1[]" required>
                                <div class="help-block with-errors"></div>
                                @if($errors->first('notecc1'))
                                <span class="text-red-700">{{ $errors->first('notecc1') }}</span>
                                @endif
                                </td>
                                <td class="text-center px-4">
                                <input type="number" step="any" value="00.0" max="20" pattern="[0-9]+([\.][0-9]+)?" class="form-control" placeholder="Saisir le note CC sur 20" id="s{{ $key+1 }}" name="notecc2[]" required>
                                <div class="help-block with-errors"></div>
                                @if($errors->first('notecc2'))
                                <span class="text-red-700">{{ $errors->first('notecc2') }}</span>
                                @endif
                                </td>
                            </tr>
                        @endforeach 
