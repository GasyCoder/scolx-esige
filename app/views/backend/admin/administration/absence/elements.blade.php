                ...
                <table class="table-sorter table table-auto table_hoverable w-full">
                    ...
                    <?php $show = Absence::where('element_id', $element->id)->first()?>
                        <tr>
                           <td>{{$key+1 }}</td>
                           <td class="font-bold">
                            <a href="{{ URL::current().'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($element->codeEc), '+/', '-_'), '=') }}/absence/take">{{$element->name }}</a> 
                            </td>
                            <td class="text-center">
                                @if(!empty($show->element_id))
                                <div class="inline-flex ltr:ml-auto rtl:mr-auto">
                                 <a href="{{URL::route('voirlist', [$class->id, $parcour->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=')])}}" class="badge badge_success" style="color:#fff">
                                ...
                                @endif
                                <div class="inline-flex ltr:ml-auto rtl:mr-auto">
                                 <a href="{{ URL::current().'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($element->codeEc), '+/', '-_'), '=') }}/absence/take" class="badge badge_primary" style="color:#fff">
...
<div class="alert alert_primary">
    <strong class="uppercase"><bdi>Désolé!</bdi></strong>
     Il n'y a pas des <u><a href="{{URL::route('indexEc')}}" style="color:#EF4444">éléménts constitutifs</a></u> sur ces niveau et parcour.  
        ...
        <hr>
         <div class="flex flex-col gap-y-5 p-5">
            ...
            <a href="#" class="flex items-center text-normal">
                <span class="la la-check-circle text-2xl leading-none ltr:mr-2 rtl:ml-2"></span>
                Tronc commun
                <span class="badge badge_danger ltr:mr-2 rtl:ml-2 ltr:ml-auto rtl:mr-auto">Non</span>
            </a>
        </div>
    </aside>
@stop