             ...
             <div class="breadcrumb breadcrumb_alt p-5">   
                ...
                <div class="card p-5 mt-5">
                    <table class="table-sorter table table_bordered w-full mt-0">
                        <thead>
                            <tr style="color:#334155">
                                ...
                                <th class="" style="text-align:center;">Parccours</th>
                        ...
                        @endforeach 
                        <?php $matieres = EC::where('tronc', 1)->where('status', 1)->where('class_id', $class->id)->where('parcour_id', '!=', $parcour->id)->where('codeEc', $element->codeEc)->get(); ?> 
                        @if(count($matieres) > 0)
                        @foreach($students_tronc_commun as $key => $student) 
                            <tr>
                                <td class="text-center">
                                <center>
                                <div class="avatar w-4 h-4">
                   ...
                   <div class="mt-auto">
                    ...
                    ?>
                    @if(count($absences) > 0)    
                        <a href="{{URL::route('voirlist', [$class->id, $parcour->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=')])}}" class="btn btn_success">    
