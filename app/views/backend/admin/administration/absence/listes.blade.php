...
@include('backend.break')
        ...
        <div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2 mt-2">  
            <a href="{{ URL::to('/admin/absences/etudiants').'/'.$class->id.'/'.$parcour->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($element->codeEc), '+/', '-_'), '=').'/absence/take'}}" class="btn btn-icon btn-icon_large btn_outlined btn_primary">
            ...
            <div class="custom-select mt-0">
                      <select name="select_element_name" id="select_element_name" onchange="goToNext(this)" class="form-control">
                        ...
                        @foreach($elements as $mat)
                            <option value="{{ URL::to('/admin/absences/etudiants').'/'.$mat->class_id.'/'.$mat->parcour_id.'/'.rtrim(strtr(base64_encode($mat->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($element->codeEc), '+/', '-_'), '=')}}/absence/take">{{ $mat->name }}</option>
                        @endforeach
                        </select>
                    <div class="custom-select-icon la la-caret-down"></div>
                </div>
