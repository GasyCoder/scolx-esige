...
@include('backend.break')
        ...
        <div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2 mt-2">  
            <a href="{{ URL::to('admin/absences/etudiants_tronc').'/'.$class->id.'/'.rtrim(strtr(base64_encode($element->id), '+/', '-_'), '=').'/'.rtrim(strtr(base64_encode($element->codeEc), '+/', '-_'), '=').'/absence_tronc/add'}}" class="btn btn-icon btn-icon_large btn_outlined btn_primary">
            ...
            <div class="custom-select mt-0">
                      <select name="select_element_name" id="select_element_name" onchange="goToNext(this)" class="form-control">
                        ...
                        @foreach($elements as $mat)
                            <option value="{{ URL::to('/admin/absences/etudiants_tronc').'/'.$mat->class_id.'/'.rtrim(strtr(base64_encode($mat->id), '+/', '-_'), '=')}}">{{ $mat->name }}</option>
                        @endforeach
                        </select>
                    <div class="custom-select-icon la la-caret-down"></div>
                </div>
      ...
      <div class="flex flex-col gap-y-5 ">
            <div class="card relative p-5">
                <table class="table-sorter table table_bordered w-full mt-3">
                        ...
                        <tbody>
                           @foreach($absencesTronc as $key => $absence)  
                            <?php 
    ...
    <aside class="sidebar">
        ...
        <div class="flex flex-col gap-y-5 p-5">
            ...
            <a href="#" class="flex items-center text-normal">
                <span class="la la-check-circle text-2xl leading-none ltr:mr-2 rtl:ml-2"></span>
                Matière
                <span class="badge badge_danger ltr:mr-2 rtl:ml-2 ltr:ml-auto rtl:mr-auto">Tronc commun</span>
            </a>
            <a href="#" class="flex items-center text-normal">
