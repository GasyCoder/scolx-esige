...
@section('content')
    <main class="workspace">
            ...
            <div class="breadcrumb breadcrumb_alt p-3 flex flex-wrap gap-2">
                ...
                <a href="{{URL::route('deliberation_1', [$class->id, $parcour->id])}}" class="btn btn_success uppercase">
                ...
                @else
                    ...
                    <a onclick="return confirm('Vous êtes sur cette action?')" href="{{URL::route('restore_s1', [$class->id, $parcour->id])}}" class="btn btn_danger uppercase">
                        <span class="la la-trash-restore text-xl leading-none"></span>
                        Restaurer
                    </a>
                    @endif @endif
                @endif
                ...
                <table class="table-sorter table w-full mt-3">
                    <thead>
                        <tr>
                            ...
                            @if($class->short == 'L1')
                            <th class="ltr:text-left rtl:text-right" width="80">Moyenne S_1</th>
                            @elseif($class->short == 'L2')
                            <th class="ltr:text-left rtl:text-right" width="80">Moyenne S_3</th>
                            @elseif($class->short == 'L3')
                            <th class="ltr:text-left rtl:text-right" width="80">Moyenne S_5</th>
                            @elseif($class->short == 'M1')
                            <th class="ltr:text-left rtl:text-right" width="80">Moyenne S_7</th>
                            @elseif($class->short == 'M2')
                            <th class="ltr:text-left rtl:text-right" width="80">Moyenne S_9</th>
                            @endif
                    ...
                    <?php      
                        ...
                        ?>     
                            <tr>
                                ...
                                @if($totalponder > 0)
                                <td class="font-bold">
                                    <?php $moyenes_One = ($totalponder/$summeCoef); ?>
                                    @if($moyenes_One >= 10)
                                    <span class="" style="color:green">{{number_format($moyenes_One, 2, '.', '')}}</span>
                                    @elseif($moyenes_One >= 9.50 && $moyenes_One <= 9.99)
                                    <span class="" style="color:blue">{{number_format($moyenes_One, 2, '.', '')}}</span>
                                    @elseif($moyenes_One <= 9.49)
                                    <span class="" style="color:red">{{number_format($moyenes_One, 2, '.', '')}}</span>
                                    @endif
                                 </td>
                                @else
                            ...
                            @if($presents > 0)         
                                ...
                                @else
                                <a href="/admin/detailnotes/semestre_1/niveau/etudiants/{{$check->class_id.'/'.$check->parcour_id.'/'.$check->id_student}}" target="_blank" class="badge badge_primary" style="color:#fff">
                                    <span class="la la-eye text-xl leading-none ltr:mr-2 rtl:ml-2"></span>
                                    Details
                                </a>
                                @endif
