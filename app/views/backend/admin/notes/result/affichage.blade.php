...
<head>
    ...
    <title>Mode impression - Résultats d'examen </title>
    <!-- Generics -->
...
<hr>
    <div class="px-7 py-5 flex justify-center">
        ...
        <p class="font-bold">Liste des admis en 1<sup>ère</sup> Session</p>
    ...
    <?php 
        $realPay = Pay::where('id_etudiant',    $affichage->id_student)
                                ->where('class_id',     $affichage->class_id)
                                ->where('parcour_id',   $affichage->parcour_id)
    ...
    @if($realPay == 10)       
      <tr>
        ...
        <td>
            @if($affichage->moyenne >= 10 && $affichage->moyenne <= 11.99)
                <span class="text-blue-700">Passable</span> 
                @elseif($affichage->moyenne >= 12 && $affichage->moyenne <= 13.99)
                <span class="text-blue-700">Assez-Bien</span> 
                @elseif($affichage->moyenne >= 14 && $affichage->moyenne <= 15.99) 
                <span class="text-green-700">Bien</span> 
                @elseif($affichage->moyenne >= 16) 
